...
class Mail
{
    ...
    public function preSend()
    {
        try {
            ...
            if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
                throw new Mail\MailerException($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty and empty($this->Body)) {
                throw new Mail\MailerException($this->lang('empty_message'), self::STOP_CRITICAL);
            }
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                if (!@$mail = popen($sendmail, 'w')) {
                    throw new Mail\MailerException($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new Mail\MailerException($this->lang('instantiate'), self::STOP_CRITICAL);
        }
        return true;
    }
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            throw new Mail\MailerException($this->lang('connect_host'));
                        }
                    ...
                    if ($this->SMTPAuth) {
                        if (!$this->smtp->authenticate(
                            ...
                            throw new Mail\MailerException($this->lang('authenticate'));
                        }
                    }
                    return true;
                } catch (phpmailerException $exc) {
        ...
        if ($this->isError()) {
            ...
            try {
                ...
                if (false === file_put_contents($file, $body)) {
                    throw new Mail\MailerException($this->lang('signing') . ' Could not write temp file');
                }
                ...
                if (@openssl_pkcs7_sign(
                    ...
                    @unlink($file);
                    @unlink($signed);
                    throw new Mail\MailerException($this->lang('signing') . openssl_error_string());
                }
            } catch (phpmailerException $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment')
    {
        try {
            if (!@is_file($path)) {
                throw new Mail\MailerException($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function encodeFile($path, $encoding = 'base64')
    {
        try {
            if (!is_readable($path)) {
                throw new Mail\MailerException($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
    ...
    public function set($name, $value = '')
    {
        try {
            if (isset($this->$name)) {
                ...
                throw new Mail\MailerException($this->lang('variable_set') . $name, self::STOP_CRITICAL);
            }
        } catch (Exception $exc) {
            $this->setError($exc->getMessage());
